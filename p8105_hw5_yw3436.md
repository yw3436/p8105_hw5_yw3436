...
mu. This is because for tests in which the null is rejected, the random
error is larger in these samples. Therefore, if we only calculate the
estimated mu for these samples, there will be large random error so that
the estimated mean will be away from the true mean.
